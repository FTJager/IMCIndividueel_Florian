...
void qwiic_twist_task(void* pvParameters)
{
    ...
    while (config->task_enabled) {
		...
		ESP_LOGI("DEBUG" ,"TASK running");
		//xSemaphoreTake( config->xMutex, portMAX_DELAY );
		
		ESP_LOGI("DEBUG" ,"Get status, %d", qwiic_twist_get_status(config, &result));
		...
		ESP_LOGI("DEBUG" ,"RESULT: %d", result);
...
esp_err_t qwiic_twist_start_task(qwiic_twist_t* config) {
	...
	config->task_time = 0;		//Debugging line
	if (config->task_time == 0) {
		config->task_time = 100;
	}
	ESP_LOGI(TAG, "Starting task");
	xTaskCreate(qwiic_twist_task, "qwiic_twist_task", 1024 * 2, (void*)config, 10, NULL);
	ESP_LOGI(TAG, "Task started");	//I swear to fucking god this LOG is genuinely necessary for this piece of garbage to run
	return ESP_OK;
}
